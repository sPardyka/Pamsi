...
public class Board {
    ...
    private Result findBestMove(Board b, int movingSign, int alpha, int beta){
            ...
            for(int i=1;i<=boardWidth;i++){
                for(int j=1;j<=boardHeight;j++){
                    if(board[i][j].getSign()==0){
                        ...
                        result.score=findBestMove(b,PLAYER_O,alpha,beta).score;
                        bestMove=Math.max(bestMove,result.score);
            ...
            for(int i=1;i<=boardWidth;i++){
                for(int j=1;j<=boardHeight;j++){
                    if(board[i][j].getSign()==0){
                        ...
                        result.score=findBestMove(b,PLAYER_X,alpha,beta).score;
                        bestMove=Math.max(bestMove,result.score);
    ...
    public void aiMakeMove(Board b){
        Result result = b.findBestMove(b,PLAYER_O,Integer.MIN_VALUE,Integer.MAX_VALUE);
        int[] cords = new int[2];
        cords[0]=result.x;
        cords[1]=result.y;
